...
class EnhancedForecasterWithOutliers:
    """Enhanced forecasting system that distinguishes between legitimate holidays and data entry errors"""
    ...
    def _get_product_forecast(self, product: str, day: str, target_year: int, target_week: int, 
                            window: int = 8) -> float:
        """Get forecast for a specific product using enhanced methods"""
        ...
        # For legitimate holidays, use conservative forecasting
        if is_holiday:
            ...
            # Use only the most recent data and be conservative
            recent_data = product_data.tail(3)
            if len(recent_data) == 0:
                return 0.0
            
            # Use minimum of recent demand with holiday reduction
            return max(0, recent_data['boxes'].min() * 0.6)
    ...
    def forecast_weekday(self, day: str, target_year: int, target_week: int, 
        ...
        """Generate enhanced forecast for a weekday"""
    ...
    def get_forecast_summary(self) -> Dict:
        """Get summary of enhanced forecasting"""
        return {
...
def test_enhanced_forecaster():
    """Test the enhanced forecaster with outlier detection"""
    print("=== TESTING ENHANCED FORECASTER WITH OUTLIER DETECTION ===")
    ...
    forecaster = EnhancedForecasterWithOutliers(history_df)
    
    # Get summary
    summary = forecaster.get_forecast_summary()
    print(f"\nEnhanced Forecasting Summary:")
    ...
    if summary['outlier_days_list']:
        print(f"\n🚫 Outlier Days Detected:")
        for outlier in summary['outlier_days_list']:
            print(f"    {outlier[2]} Week {outlier[1]}, {outlier[0]}")
    ...
    # Test on Week 32 (holiday week with outliers)
    print(f"\n--- Testing Week 32, 2025 (Holiday Week with Outliers) ---")
    for day in ['Mon', 'Tues', 'Wed', 'Thurs', 'Fri']:
