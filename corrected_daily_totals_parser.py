# corrected_daily_totals_parser.py
...
def parse_daily_totals_correctly(file_path: Path, sheet: str) -> pd.DataFrame:
    """Parse ALL columns in the Daily Totals section, not just Walmart"""
    ...
    header_cells = [str(cell) for cell in df.iloc[header_row].values if pd.notna(cell)]
    ...
    for idx in range(header_row + 1, daily_totals_end + 1):
        ...
        # Look for product/quantity patterns in this row
        # The structure appears to be: [pieces, product1, qty1, product2, qty2, product3, qty3, ...]
        
        # Extract all products and quantities from this row
            ...
            try:
                ...
                if qty > 0:  # Only positive quantities
                    ...
                    if i + 1 < len(row_values):
                        ...
                        if product and not product.isdigit() and product.lower() not in ['pcs', 'case', 'cases', 'ea', 'each', 'pk', 'pack', 'pks', 'box total', 'our compliments']:
                            products_and_quantities.append((product, qty))
                ...
                # Not a number, might be a product name
                if (val and 
                    not val.isdigit() and 
                    ...
                    # This might be a product name, look for quantity in next cell
                    if i + 1 < len(row_values):
                        try:
                            next_qty = float(row_values[i + 1])
                            if next_qty > 0:
                                products_and_quantities.append((val, next_qty))
                                i += 2
                            else:
                                i += 1
                        except ValueError:
        ...
        # Add all found products to records
...
def test_corrected_parser():
    """Test the corrected parser on Week 24, 2025 Monday"""
    test_file = Path("Week 24 Loading Slip 2025.xlsx")
    if not test_file.exists():
        print(f"Test file {test_file} not found")
        return
    
    print("=== TESTING CORRECTED DAILY TOTALS PARSER ===")
    print(f"Testing file: {test_file}")
    
    result = parse_daily_totals_correctly(test_file, "Mon")
