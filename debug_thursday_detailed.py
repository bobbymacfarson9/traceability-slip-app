...
def debug_thursday_detailed():
    """Debug Thursday parsing in detail"""
    print("=== DEBUGGING THURSDAY IN DETAIL ===")
    ...
    try:
        ...
        # Show first few data rows in detail
        print(f"\n--- Data Rows Details ---")
        for idx in range(header_row + 1, min(header_row + 5, daily_totals_end + 1)):
            row = df.iloc[idx]
            ...
            _test_thurs_parsing_detailed(row_values, idx)
        
    except Exception as e:
        print(f"Error: {e}")

def _test_thurs_parsing_detailed(row_values, idx):
